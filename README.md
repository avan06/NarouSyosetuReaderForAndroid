# Narou Syosetu Reader For Android
...
📥 **APKダウンロード：** [NarouSyosetuReader-1.2.0.zip](https://github.com/avan06/NarouSyosetuReaderForAndroid/releases/download/1.2.0/NarouSyosetuReader-1.2.0.zip)
...
* 📚 **WebViewブラウズモード：**
  ...
  * <img width="422" height="892" alt="image" src="https://github.com/user-attachments/assets/8bb67441-5c60-47e7-bcbb-6b09ccbe364f" />

* 💾 **オフラインダウンロードと読書：**

  * 好きな小説を全巻ダウンロードして、ネット接続なしでも読書可能。
  * ダウンロード済み小説は書庫に整理して表示。
  * <img width="422" height="892" alt="image" src="https://github.com/user-attachments/assets/7d1f2c9e-8e62-4346-b060-38761c22ba17" />
  * <img width="422" height="892" alt="image" src="https://github.com/user-attachments/assets/44036244-a8a7-4798-8324-1de999a5577b" />
...
* 📖 **高度にカスタマイズ可能な読書画面：**
  ...
  * **前書き・後書き：** 設定から作者の前書きや後書きの表示・非表示を自由に切り替え可能。
  * ![image](https://github.com/user-attachments/assets/4e520e20-f097-47f7-afe1-62de42d2a136)
  * <img width="422" height="892" alt="image" src="https://github.com/user-attachments/assets/1622151e-558f-41f9-93c4-eb5a9243b748" />

* 🗣️ **TTS（テキスト読み上げ）：**

  * 目を休めながら耳で楽しむ。画面上の見える段落から音声読み上げ開始。
  * バックグラウンド再生に対応し、画面をオフにした状態でも途切れることなく連続再生が可能。
  * **スマートな音声制御：** イヤホンの切断時や着信、他アプリでのメディア再生時に自動で一時停止します。
  * 読み上げ速度調整、音声エンジン・言語選択可能。
  * 章を切り替えた際の再生位置の自動リセットや、設定から「次の話を自動で読み上げる」オプションを有効にすることでシームレスな連続再生にも対応。
  * 自動スクロール＆ハイライト表示付きで読み上げに対応。
  * **カタカナ読み上げ補正：** 外来語の不自然なアクセントを防ぐため、カタカナをひらがなに変換して読み上げるオプションを搭載。
  * **音声カスタム辞書：** 漢字の読み間違いがある場合、好みの読み方に修正できる専用の辞書登録・管理機能に対応。
  * <img width="422" height="892" alt="image" src="https://github.com/user-attachments/assets/3c87feeb-4750-4823-8e79-75f7d494b17a" />
  * <img width="422" height="892" alt="image" src="https://github.com/user-attachments/assets/b9e3c9a1-8c4e-4402-bff8-87cabc468cbc" />
...
* 🔄 **更新確認と差分ダウンロード：**
  ...
  * ホーム画面を開く際に、小説の更新が必要かどうかを自動で確認。
  * ホーム画面上部の「すべて更新」ボタンで、表示中の小説をまとめて順次更新可能。
...
* 📥 **データのバックアップと復元：**
  ...
  * バックグラウンドでダウンロードを確実に行うため、省電力ポリシーの変更（「制限なし」推奨）を案内する設定を追加。
  * <img width="422" height="892" alt="image" src="https://github.com/user-attachments/assets/4f947646-53bc-4841-8be9-eee86a98870d" />
...
📥 **APK 下載：** [NarouSyosetuReader-1.2.0.zip](https://github.com/avan06/NarouSyosetuReaderForAndroid/releases/download/1.2.0/NarouSyosetuReader-1.2.0.zip)
...
* 📖 **高度可自訂的閱讀界面 (ChapterReaderScreen)：**
  ...
  * **前言與後記：** 可在設定中自由選擇是否顯示作者的前言與後記。

* 🗣️ **TTS 語音朗讀 (文字轉語音)：**
  * 解放雙眼，聆聽故事。支援從當前螢幕可見段落開始朗讀。
  * 支援背景播放服務，即使關閉螢幕也能無縫解析並繼續聆聽下一章。
  * **智慧音訊控制：** 支援音訊焦點，遇到來電、耳機斷線或開啟其他影音 App 時會自動暫停朗讀。
  * 可調整語速，並在支援的設備上選擇不同的語音模型和朗讀語言。
  * 切換章節時會自動正確重置播放位置，並可在設定中開啟「自動朗讀下一話」達成無縫連續播放。
  * 播放/暫停控制，朗讀時自動高亮當前段落並滾動。
  * **片假名發音修正：** 提供將片假名轉換為平假名朗讀的選項，解決 TTS 引擎對外來語語調不自然的問題。
  * **自訂語音字典：** 遇到漢字或特殊詞彙發音不正確時，可透過專屬的管理介面加入「原文與發音」的對應規則進行替換。
...
* 🔄 **更新檢查與增量下載：**
  * 開啟首頁時會自動檢查已下載小說是否有新章節或內容更新。
  * 首頁上方新增「全部更新」按鈕，可一鍵將畫面上顯示的小說依序完成更新。
  * 只下載有變動的部分，節省您的時間和數據。

* 📥 **數據備份與還原：**
  * 提供將所有下載的小說內容、閱讀進度及應用程式設定匯出為 JSON 檔案的功能。
  * 支援從備份檔案匯入數據，輕鬆在不同設備間轉移或恢復您的書庫。
  * 設定中新增省電策略變更引導（建議設為「無限制」），確保背景下載順利完成。
...
📥 **Download APK:** [NarouSyosetuReader-1.2.0.zip](https://github.com/avan06/NarouSyosetuReaderForAndroid/releases/download/1.2.0/NarouSyosetuReader-1.2.0.zip)
...
* 📖 **Highly Customizable Reader (ChapterReaderScreen):**
  ...
  * **Preface & Afterword:** Toggle the visibility of the author's preface and afterword in the settings.

* 🗣️ **TTS (Text-to-Speech) Narration:**

  * Listen to stories from visible paragraphs on screen.
  * Implemented background playback, allowing seamless parsing and continuous narration even with the screen turned off.
  * **Smart Audio Control:** Automatically pauses narration upon receiving calls, disconnecting earphones, or playing media from other apps.
  * Customize speed and choose voice models or languages (if supported).
  * Accurately resets playback position when manually switching chapters, and features an optional "Auto-read next chapter" setting for seamless continuous listening.
  * Highlights and scrolls along with narration for enhanced clarity.
  * **Katakana Pronunciation Correction:** Option to convert Katakana to Hiragana before TTS to prevent unnatural foreign-loanword intonations.
  * **Custom TTS Dictionary:** Correct mispronunciations by easily managing and adding custom rules for specific words or Kanji via a dedicated interface.
...
* 🌟 **Favorites & Sorting:**
  ...
  * Mark novels as favorites for prioritized display in your library.
  * Sort by recent reading time or favorite status.

* 🔄 **Update Check & Incremental Download:**

  * Automatically checks if novels need updating when you open the Home screen.
  * Added an "Update All" button at the top of the Home screen to sequentially update all displayed novels.
...
* 📥 **Backup & Restore:**
  ...
  * Added a battery optimization guide in settings (recommending "Unrestricted") to ensure reliable background downloading.
...
### 1.2.1
1. **TTS（テキスト読み上げ）**
   * 設定画面に「読み上げ前にカタカナをひらがなに変換」オプションを追加しました（デフォルトはOFF）。カタカナの外来語読みによる不自然な発音を防ぎ、より自然な日本語のイントネーションで読み上げ可能になります。
   * 「音声カスタム辞書」のUIを刷新しました。設定画面にすべて展開する方式から、専用の全画面管理ダイアログへ変更し、登録済みルールの「検索機能」を追加しました。
   * バックグラウンド再生中の安定性を向上させました。画面オフ時でも、次の章のテキストをバックグラウンドで解析し、途切れることなく連続再生できるようになりました。
   * オーディオフォーカスの制御に対応しました。イヤホンの切断時（Bluetooth接続切れなど）や、着信・他アプリでのメディア再生時に、自動的にTTS再生が一時停止するよう改善しました。
2. **システム・その他**
   * 起動時の読み込み画面（スプラッシュスクリーン）のテキストが多言語に対応していなかった問題を修正しました。
   * アプリ起動時の動画広告（App Open Ad）をデフォルトでミュート（消音）に設定し、予期せぬ大きな音で読書体験が損なわれないよう改善しました。

### 1.2.0
1. **ダウンロード機能**
   * 小説ダウンロードの安定性を改善し、失敗時には自動で再試行するようになりました（最大2回）。
   * ダウンロード中のステータスメッセージ表示をより分かりやすく改善しました。
   * 小説のダウンロード中であっても、詳細画面の目次内容がバックグラウンドで自動更新されるように改善しました。
2. **リーダー画面**
   * 設定画面に、小説の「前書き（プレフェイス）」と「後書き（アフターワード）」を表示するかどうかのオプションを追加しました（デフォルトは表示）。
   * 読書画面を開く際、スクロール位置の復元による画面のちらつきを防ぐため、最初に本文を透明にし、250ミリ秒後にフェードイン表示するよう滑らかに改善しました。
   * 読書進捗（スクロール位置）が正しく保存されない場合がある問題を修正しました。
3. **TTS（テキスト読み上げ）**
   * TTS設定画面に「音声カスタム辞書」機能を追加しました。漢字の読みが正しくない場合などに、任意の原文と読みの対応を登録し、読み上げ時に自動置換できるようになりました。
   * 「次の話を自動再生」有効時、再生中に「前の行」ボタンを押しても前章に戻らない問題を改善し、前章の保存済み読書位置（通常は最終行）からシームレスに再生されるように修正しました。
   * TTS進捗が不適切に初期化される問題を修正しました。小説または章が実際に変更された場合のみ読書位置をリセットするように改善しました。
   * 設定のインポート時に、「次の話を自動再生」の設定値が正しく復元されない不具合を修正しました。
4. **システム**
   * 最新のAndroid OS（Target SDK 36）への互換性を向上させました。

### 1.1.0
1. 設定画面に、省電力ポリシーの変更を案内するオプションを追加しました。バックグラウンドで小説のダウンロードを完了させたい場合は、「制限なし」に変更することをおすすめします。
2. ホーム画面を開く際に、小説の更新が必要かどうかを自動で確認するようになりました。また、ホーム画面上部に「すべて更新」ボタンを追加し、表示中の小説をまとめて順次更新できるようになりました。
3. TTS用のバックグラウンド再生サービスを実装しました。これにより、画面をオフにした状態でもTTSの再生が可能になりました。
4. TTS再生中に章を切り替えた場合でも、再生位置が正しくリセットされ、自動的に最初から読み上げが行われるようになりました。
5. TTS設定画面に「次の話を自動で読み上げる」オプションを追加しました（初期状態ではオフ）。有効にすると、現在の章の読み上げ終了時に自動的に「次の話」へ進み、TTSの再生位置もリセットされて最初から読み上げが開始されます。

### 1.0.4
小説を更新した後、目次が最新話を正しく表示するように修正しました。
